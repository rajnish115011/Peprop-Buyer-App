...
class All_transactions_Tab extends StatefulWidget{
  @override
...
class Build_state extends State{
  ...
  Future<void> get_data() async {
    ...
    if(response.data!=null&&response.statusCode==200){
      var notify = response.data["data"]["CREDIT"] as List;
      list = notify.map<All_transactions_Model>((json) =>All_transactions_Model.fromJson(json)).toList();
      var debit = response.data["data"]["DEBIT"] as List;
      list.addAll(debit.map<All_transactions_Model>((json) =>All_transactions_Model.fromJson(json)).toList());
      var book_now = response.data["data"]["BOOKNOW"] as List;
      list.addAll(book_now.map<All_transactions_Model>((json) =>All_transactions_Model.fromJson(json)).toList());
      var bbps = response.data["data"]["BBPS"] as List;
      list.addAll(bbps.map<All_transactions_Model>((json) =>All_transactions_Model.fromJson(json)).toList());
      list.sort((a,b) {
