...
class My_Gift_Card extends StatefulWidget{
...
class Build_State extends State<My_Gift_Card>{
      ...
      children: [
        if(loading)Center(child: new CircularProgressIndicator(),)
          ...
          children: [
            ...
            if(my_giftcards.length>0)Widget_Gift_Grid(my_giftcards,true,section_banner,false)else No_Data_Placeholder("No Gift Card Available.!!"),
  ...
  get_campaign() async {
    ...
    if(response!=null&&response.data!=null&&response.statusCode==200&&response.data["list"]!=null){
      ...
      if(response.data["list"]["my_giftcards"]!=null){
        var notify = response.data["list"]["my_giftcards"] as List;
