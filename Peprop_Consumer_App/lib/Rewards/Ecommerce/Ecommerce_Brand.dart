...
class Ecommerce_Brand extends StatefulWidget{
  late Ecommerce_Model categeory;
  Ecommerce_Brand(Ecommerce_Model categeory){
...
class Build_State extends State<Ecommerce_Brand>{





  List<Brand_Model>categeory=[];
  List<Brand_Model>all_categeory=[];


  @override
  void initState() {
    super.initState();
    categeory.addAll(widget.categeory.brand_list);
    all_categeory.addAll(widget.categeory.brand_list);
  ...
  Widget build(BuildContext context) {
    ...
    return new
    Scaffold(
      appBar: AppUtils.appBarTitle(context, "Brands"),
                 ...
                 itemBuilder: (BuildContext context, int index) {
                  return new InkWell(
                    child:
                      ...
                      children: [
                        Expanded(child: new Container(decoration: new BoxDecoration(borderRadius: new BorderRadius.circular(10),color: ColorFile.tabback,),child: new Column(
                          children: [
                            new Image.network(categeory[index].logo,height: 120,),
                            new Container(padding: EdgeInsets.only(left: 5,right: 5),child:new Text(categeory[index].name,style: Styles().styles_medium(15, ColorFile.hint_color),),height: 40,alignment: Alignment.centerLeft,)
                          ],
                        ))),
                      ...
                      if(categeory[index].varient_list.length>0){
                        Navigator.of(context).push(MaterialPageRoute(builder: (BuildContext context) => Ecommerce_Varient(categeory[index])));
