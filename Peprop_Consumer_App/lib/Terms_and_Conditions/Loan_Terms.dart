...
class Loan_Terms extends StatefulWidget {
  @override
...
class Build_Statess extends State{
  ...
  Future<void> get_tnc() async {
    ...
    await  FireBase_Logs().log_screen_view("View App Loan Terms", "View App Loan Terms");
    setState(() {
      loading=true;
    });
    ...
    if(response!=null&&response.data!=null&&response.statusCode==200&&response.data["data"]!=null){
      tnc = response.data["data"]["loan_tnc"].toString().replaceAll("null", "");
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppUtils.appBarTitle(context, "Loan Terms & Conditions"),
      body: Stack(
        children: [
